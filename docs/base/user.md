...
- Через `Member.personId` учётка может быть привязана к узлу дерева (`Person`) внутри семьи. См. `docs/base/user-member-person.md` и `docs/family/entities.md`.

## Замечания по модели
- Роли и права вычисляются на основе роли из `Member` активной семьи. См. `docs/base/roles.md`, `docs/base/permissions.md`.
- Идентификаторы внешних провайдеров (например, `telegramId`) используются для верификации в `docs/authorization/README.md`.
