...
- Через `Member.personId` учётка может быть привязана к узлу дерева (`Person`) внутри семьи. См. `./user-member-person.md` и `../family/entities.md`.

## Замечания по модели
- Роли и права вычисляются на основе роли из `Member` активной семьи. См. `./roles.md`, `./permissions.md`.
- Идентификаторы внешних провайдеров (например, `telegramId`) используются для верификации в `../authorization/README.md`.
