...
- Через `Member.personId` учётка может быть привязана к узлу дерева (`Person`) внутри семьи. См. `./identity.md` и `./member.md`.

## Замечания по модели
- Роли и права вычисляются на основе роли из `Member` активной семьи. См. `../roles.md`, `../permissions.md`.
- Идентификаторы внешних провайдеров (например, `telegramId`) используются для верификации в `../../guides/authorization/README.md`.
