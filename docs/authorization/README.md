...
Сущность `User` описана в `../base/user.md`.

Ключевые моменты для авторизации:
- Права и роли выводятся из `Member` активной семьи (см. `../base/roles.md`, `../base/permissions.md`).
- Привязка к узлу дерева выполняется через `Member.personId` (см. `../base/user-member-person.md`).
