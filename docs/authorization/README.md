...
5. Сервис находит или создаёт пользователя и активную сессию.

#### 3.1.1 Новый владелец семьи
- Пользователь пришёл без `invitationId`.
- После подтверждения создаётся новая семья и пользователь получает роль `owner`.
- Фронт получает `accessToken` и `familyId` новой семьи.

#### 3.1.2 Новый приглашённый участник
- Пользователь переходит по ссылке с `invitationId`.
- После подтверждения создаётся учётная запись и членство в семье с указанной ролью.
- Фронт получает `accessToken` и активную семью пригласившего.

#### 3.1.3 Существующий пользователь
- Учётная запись уже существует.
- После подтверждения обновляется сессия и возвращается `accessToken`.
- Активная семья определяется последней использованной или выбрана отдельно.
...
Базовая сущность учётной записи пользователя, используемая для аутентификации и связи с внешними провайдерами. Роли и права не хранятся в `User`, а определяются на уровне членства (`Member`) в конкретной семье.
...
Примечания:
- Актуальные права вычисляются из `Member` выбранной активной семьи (см. `../base/roles.md`, `../base/permissions.md`).
- Привязка к узлу дерева возможна через `Member.personId` (см. `../base/user-member-person.md`).
