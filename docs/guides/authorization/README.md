...
Клиент использует refresh‑токен (httpOnly cookie); сервер выдаёт новый access‑JWT и ротирует `jti`.
...
7. Сгенерировать `jti`, записать `sess:{jti}` и подготовить `AuthResponse` для выдачи на фронт при опросе `GET /auth/telegram/session/{sid}`; выдать refresh‑cookie.
